...
def generate_reports(G):
    ...
    weighted_degree = nx.degree_centrality(G)
    closeness = nx.closeness_centrality(G)
    cent_df = pd.DataFrame.from_dict([betweenness, weighted_degree, closeness, simple_degree])
    cent_df = pd.DataFrame.transpose(cent_df)
    cent_df.columns = ['betweenness', 'weighted degree', 'closeness', 'simple_degree']
    cent_df['node'] = cent_df.index
    cent_df = cent_df[['node', 'betweenness', 'weighted degree', 'closeness', 'simple_degree']]
    return cent_df
    ...
    return html.Div([
        ...
        html.Div(
            html.H3(
                ...
                style={'textAlign': 'center', 'color': 'black', 'padding-top':20, 'padding-bottom':20}
                )
            ),
        html.Div(
            ...
            children=[
                ...
                dcc.Input(id='datafile', value=os.getcwd(), type='text', style={'size':50, 'padding-top':20, 'padding-bottom':20})],
                style={'float': 'right'}
            ),
        html.Div(
                ...
                dcc.Tabs(id='tabs_with_classes', value='tab_1', parent_className='custom_tabs', className='custom_tabs_container',
                    children=[
                        dcc.Tab(label='Workflow', value='tab_1', className='custom_tab', selected_className='custom_tab__selected',
                            children=[html.Div(
                                dcc.Markdown("""
                                    This dashboard is compatible with data pulled from Web of Science (WoS).  

                                    Folders paths or single file paths can be input into the text box above.  

                                    The 'Get data' button will load and prepare the data for network analyses and visualization.  

                                    For details on workflow process, and documentation, please visit our GitHub.

                                    """),
                                        style={'textAlign':'left', 'font-size':'18px', 'padding-top':50, 'padding-bottom':20,
                                            'li':'before'}),

                            html.A('GitHub', href='https://github.iu.edu/rsciagli/MultiNet_Toolkit', target='_blank',
                                    style={'textAlign':'center', 'font-size':'18px', 'padding-top':20, 'padding-bottom':20,
                                        'li':'before'})
                            ]),
                        dcc.Tab(label='About', value='tab_2', className='custom_tab', selected_className='custom_tab__selected',
                            children=[html.Div(
                                dcc.Markdown("""
                                    The MultiNet Toolkit is an interactive dashboard built for IU Network Science Institute (IUNI). 
                            ...
                            html.A('IUNI', href='https://iuni.iu.edu/', target='_blank',
                                    style={'textAlign':'center', 'font-size':'18px', 'padding-top':20, 'padding-bottom':20,
                                        'li':'before'})
                            ]),
                                    ...
                                    children=[html.Div('Centrality Metrics',
                                        ...
                                        dcc.Dropdown(id='centrality_type',
                                            options=[
                                                ...
                                                {'label': 'Weighted degree', 'value': 'weighted degree'},
